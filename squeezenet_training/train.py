...
def main(args):
    ...
    extractor = (torch.hub.load('pytorch/vision:v0.4.2', 'squeezenet1_1', pretrained=True)).features
    model = Model(extractor)
    loss_fn = AgeGenderLoss(1.0, 1.0).to(device=device)
...
def train(dataloader, model, loss_fn, optimizer, device, writer, epoch, global_step, print_freq):
    age_meter = AverageMeter('Age Loss', ':.4e')
    gender_meter = AverageMeter('Gender Loss', ':.4e')

    progress = ProgressMeter(
        len(dataloader),
        [age_meter, gender_meter],
    ...
    for i, (inputs,targets) in enumerate(dataloader):
        ...
        gender_loss, age_loss = loss_fn(pred, targets)
        
        loss = age_loss + gender_loss

        writer.add_scalar('Loss/Loss', loss.item(), global_step)
        writer.add_scalar('Loss/Age', age_loss.item(), global_step)
        writer.add_scalar('Loss/Gender', gender_loss.item(), global_step)

        age_meter.update(age_loss.item(), inputs.size(0))
        gender_meter.update(gender_loss.item(), inputs.size(0))
...
def validate(model, dataloader, device, writer, epoch, global_step):
    ...
    gender_acc = AverageMeter('GenderAcc', ':6.2f')
    ...
    with torch.no_grad():
        for i, (inputs, targets) in enumerate(dataloader):
            ...
            gender_pred, age_pred = pred[:, 0], pred[:, 1]
